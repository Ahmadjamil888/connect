...
class BasePaymentAdapter(IMOSAdapter):
    def __init__(self, name: str, config: dict[str, Any] | None = None, capabilities: list[str] | None = None) -> None:
        super().__init__(name=name, adapter_type="payment", capabilities=capabilities or ["charge", "refund", "payout"], config=config)
    ...
    async def send(self, task: IMOSTask) -> IMOSResult:
        ...
        try:
            ...
            if handler is None:
                raise AttributeError(f"Unsupported payment action: {action}")
            params = dict(task.metadata.get("params", {}))
    ...
    def require_confirmation(self, confirm: bool) -> None:
        if not confirm:
            raise PermissionError("Financial operation requires confirm=True")
